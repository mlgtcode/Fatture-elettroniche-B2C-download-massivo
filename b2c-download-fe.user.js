...
// @version      1.1
// @description  Aggiunge il pulsante "Download massivo" per scaricare le fatture in sequenza
...
(function() {
    function addMassDownloadButton(exportButton) {
    ...
    function observeForExportButton() {
        const exportButton = document.querySelector('button[title="esporta la tabella"]');
        if (exportButton) {
            addMassDownloadButton(exportButton);
            observer.disconnect();
        }
    }
    const observer = new MutationObserver(observeForExportButton);
    ...
    async function massDownloadScript() {
        ...
        for (let i = 0; i < links.length; i++){
            ...
            newHref = originalHref.startsWith(prefix)
                ? "https://serviziae.agenziaentrate.gov.it/FattureWeb/fatture/dettaglio/" + originalHref.substring(prefix.length)
                : originalHref;
